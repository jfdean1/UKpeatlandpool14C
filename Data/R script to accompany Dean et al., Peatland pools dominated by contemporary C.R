...
ggdraw() +
  ...
  draw_plot_label(label = c("A", "B"), size = 15,
                  x = c(0, 0.445), y = c(1, 1))
ggsave("Fig_1.jpeg", width = 25, height = 10, units = c("cm"), dpi = 600)
...
ggsave("Fig_2.jpeg", width = 10, height = 12, units = c("cm"), dpi = 600)
...
ggsave("Fig_3.jpeg", width = 22, height = 10, units = c("cm"), dpi = 600)
...
ggsave("Fig_4.jpeg", width = 14, height = 10, units = c("cm"), dpi = 600)
...
ggsave("Fig_5.jpeg", width = 25, height = 10, units = c("cm"), dpi = 600)
...
ggsave("Fig_6.jpeg", width = 15, height = 20, units = c("cm"), dpi = 600)
...
Fig_F14Cd13Cspace = ggplot(F14C_data, aes(x = d13C, y = F14C, shape = factor(Pool_type), color = factor(C_species_n))) +
  ...
  annotate("text", x=-68, y=1.01, size=3, label=expression(italic('F'^14*'C modern')), color = "snow3") +
  geom_hline(yintercept=1, linetype="dashed", linewidth = 0.25, color = "snow3") +
  annotate("text", x=-4.2, y=1.05, size=3, label='atmospheric', color = "black") +
  annotate("text", x=-4, y=1.038, size=3, label=expression('CO'[2]*' 2013-15'), color = "black") +
  geom_rect(aes(xmin = -9.5, xmax = -8.5, ymin = 1.0234, ymax = 1.0301), linewidth = 0.15, fill = "white", alpha = 0, color = "black") +
  annotate("text", x=-38.9, y=0.983, size=3, label='modern plant material', color = "black") +
  annotate("text", x=-34.9, y=0.971, size=3, label='1950-2012', color = "black") +
  geom_rect(aes(xmin = -30, xmax = -23, ymin = 0.9740, ymax = 1.8495), linewidth = 0.15, fill = "white", alpha = 0, color = "black") +
  annotate("text", x=-47.2, y=1.115, size=3, label=expression('peak atmospheric '^14*'CO'[2]*' in 1964 = 1.8495'), color = "black") +
  geom_segment(aes(x = -31, y = 1.1, xend = -31, yend = 1.13), arrow = arrow(length=unit(.125, 'cm')), linewidth = 0.3, color = "black") +
  annotate("text", x=-36.5, y=0.85, size=3, label='0.5-1 m deep peat', color = "black") +
  annotate("text", x=-38.2, y=0.838, size=3, label='~1,000-3,000 years old', color = "black") +
...
ggsave("Fig_7.jpeg", width = 20, height = 12, units = c("cm"), dpi = 600)
...
Fig_poolarea <- ggplot(Pool_phys, aes(x=Pool_type, y=Area, fill= Pool_type)) + 
  ...
  xlab('Pool type') + ylab(expression('Pool surface area (m'^2*') [log scale]')) +
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + theme(legend.position = "none") +
  theme (axis.title=element_text(size=8), axis.text = element_text(size = 7))
Fig_poolarea
...
# new potential figure for supplement
...
ggsave("Fig_outliers.jpeg", width = 15, height = 7, units = c("cm"), dpi = 600)
